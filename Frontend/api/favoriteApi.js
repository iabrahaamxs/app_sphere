...
const addFavorite = async (jwt, postId) => {
  try {
    ...
    return { ok: false, message: error.response?.data?.message || "Error adding favorite" };
  }
};

const isFavorite = async (jwt, postId) => {
  try {
    const res = await axiosManager.get("/private/favorite/isFavorite", {
      params: { post: postId }, 
      headers: {
        Authorization: jwt, 
      },
    ...
    return { ok: false, message: error.response?.data?.message || "Error checking favorite status" };
  }
};
...
const deleteFavorite = async (jwt, postId) => {
  try {
    ...
    return { ok: false, message: error.response?.data?.message || "Error deleting favorite" };
  }
};
